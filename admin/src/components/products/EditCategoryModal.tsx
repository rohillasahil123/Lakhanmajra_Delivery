...
export default function EditCategoryModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await api.patch(`/categories/${category._id}`, fd, {
      ...
      if (newSubs.length > 0) {
        await Promise.all(
          newSubs.map((s) => {
            ...
            fd2.append('parentCategory', category._id);
              ...
              {preview && !removeImg ? (
                <div className="relative group">
                  <img
                    ...
                    onError={(e) => {
                      (e.target as HTMLImageElement).src = 'https://via.placeholder.com/56?text=No';
                    }}
                  />
                  <button
                    type="button"
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {subCategories.map((sub) => (
                  <div
                    key={sub._id}
                    className="flex items-center gap-1 bg-slate-100 rounded-full px-3 py-1 text-sm text-slate-600"
                  >
                    {sub.name}
                    <button
                      type="button"
                      className="text-slate-400 hover:text-red-500 ml-1 transition-colors leading-none"
                      onClick={() => deleteSubCat(sub._id, sub.name)}
                    >
                      ×
                    </button>
                  </div>
