...
export default function CategoryManagerModal({ onClose, onCategoriesChanged }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await api.patch(`/categories/${editingId}`, fd, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      ...
      if (subs.length > 0) {
        await Promise.all(
          subs.map((s) => {
            ...
            return api.post('/categories', fd2, {
              headers: { 'Content-Type': 'multipart/form-data' },
            });
          })
        );
      ...
      <tr key={c._id} className="border-b last:border-0 hover:bg-slate-50 transition-colors">
          ...
          {isEditing ? (
            <div className="flex items-center gap-2">
              {ePreview && !eRemoveImg ? (
                ...
                <div className="w-10 h-10 rounded-lg border-2 border-dashed border-slate-300 flex items-center justify-center text-slate-400 text-xs">
                  No
                </div>
              )}
              <div className="flex flex-col gap-1">
                ...
                {(ePreview || c.image) && !eRemoveImg && (
                  <button
                    type="button"
                    className="text-xs text-red-500 hover:text-red-700 px-2 py-1 bg-red-50 rounded transition-colors"
                    onClick={() => {
                      setERemoveImg(true);
                      setEFile(null);
                      setEPreview('');
            ...
            <div className="w-10 h-10 rounded-lg border border-slate-200 bg-slate-50 text-slate-400 text-xs flex items-center justify-center">
              No
            </div>
          )}
        </td>
        ...
        <td className="px-3 py-2.5">
          {isEditing ? (
            <input
              type="number"
              className="border px-2 py-1.5 rounded text-sm w-16"
              value={ePriority}
        ...
        <td className="px-3 py-2.5">
          {isEditing ? (
            <div className="flex gap-1.5">
              ...
              <button
                className="px-3 py-1.5 bg-slate-200 text-xs rounded hover:bg-slate-300 transition-colors"
                onClick={cancelEdit}
              >
                Cancel
              </button>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              {mainCategories.length} main · {subCategories.length} sub-categories
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-4 border-b bg-slate-50">
          ...
          <div className="flex flex-wrap gap-2 items-end">
            ...
            <input
              ...
              value={cPriority}
              onChange={(e) => setCPriority(Number(e.target.value))}
            />
              ...
              <>
                ...
                {cShowSub && (
                  <input
                    ...
                    value={cSubInput}
                    onChange={(e) => setCSubInput(e.target.value)}
                  />
                )}
              </>
            )}
            ...
            <label className="text-sm px-3 py-2 bg-slate-100 hover:bg-slate-200 rounded cursor-pointer transition-colors">
              ...
              <input
                ...
                onChange={(e) => {
                  const f = e.target.files?.[0] || null;
                  setCFile(f);
                  setCPreview(f ? URL.createObjectURL(f) : '');
                }}
              />
            </label>
            {cPreview && (
            ...
            <table className="w-full text-left">
              <thead className="sticky top-0 bg-white border-b">
                <tr className="text-xs uppercase tracking-wider text-slate-400">
                  ...
                  {activeTab === 'main' && (
                    <th className="px-3 py-2.5 font-medium">Sub-categories</th>
                  )}
