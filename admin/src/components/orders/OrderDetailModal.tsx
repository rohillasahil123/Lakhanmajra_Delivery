...
export default function OrderDetailModal({
  ...
  const subtotal =
    detail?.items?.reduce((s, i) => s + Number(i.quantity ?? 0) * Number(i.price ?? 0), 0) ?? 0;
        ...
        <div className="flex items-start justify-between px-6 py-4 border-b border-slate-100 flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              {detail
                ? `${formatDate(detail.createdAt)} · ${formatTime(detail.createdAt)} · ${timeAgo(detail.createdAt)}`
                : 'Loading...'}
            </p>
          </div>
          <button
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                <div>
                  ...
                  <p className="text-slate-500 text-xs">
                    {detail.userId?.phone || detail.userId?.email || '—'}
                  </p>
                </div>
                <div>
              ...
              {detail.items && detail.items.length > 0 ? (
                <div className="space-y-3">
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        {item.mrp && item.price && item.mrp > item.price && (
                          <p className="text-xs text-slate-400 line-through">
                            ₹{Number(item.quantity ?? 0) * item.mrp}
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="mt-2 pt-2 border-t border-slate-100 space-y-1 text-sm">
                    ...
                    <div className="flex justify-between text-slate-500">
                      <span>Delivery</span>
                      <span>₹{Number(detail.deliveryFee ?? 0).toFixed(2)}</span>
                    </div>
            ...
            <Section title="Payment" icon="💳">
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-slate-50 rounded-xl p-3 text-center">
                  ...
                  <span
                    className={`text-sm font-bold uppercase ${detail.paymentMethod === 'online' ? 'text-blue-600' : 'text-orange-600'}`}
                  >
              ...
              {detail.assignedRiderId ? (
                <div className="flex items-center gap-3 mb-3 p-3 bg-blue-50 rounded-xl border border-blue-100">
                  ...
                  <div className="flex-1">
                    <p className="font-semibold text-slate-800 text-sm">
                      {detail.assignedRiderId.name}
                    </p>
                    <p className="text-xs text-slate-500">
                      {(detail.assignedRiderId as any).phone || '—'}
                    </p>
                  </div>
                  ...
                  {statusList.map((s, idx) => {
                    ...
                    return (
                      <React.Fragment key={s}>
                        ...
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${isCurrent || editStatus === s ? m.dot : isDone ? 'bg-emerald-400' : 'bg-slate-300'}`}
                          />
                          {m.label}
                        </button>
                        {idx < statusList.length - 1 && (
                          <div
                            className={`flex-shrink-0 w-4 h-px ${isDone ? 'bg-emerald-300' : 'bg-slate-200'}`}
                          />
                        )}
                      </React.Fragment>
                    );
