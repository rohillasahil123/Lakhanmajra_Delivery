...
export type Order = {
  ...
  totalAmount: number; deliveryFee?: number;
...
export type FilterState = {
  ...
  from: string; to: string; today: boolean;
};
...
export const STATUS_META: Record<string, { label: string; bg: string; text: string; dot: string; border: string }> = {
  ...
  confirmed:  { label: "Confirmed",  bg: "bg-blue-50",    text: "text-blue-700",    dot: "bg-blue-500",    border: "border-blue-200"    },
  processing: { label: "Processing", bg: "bg-indigo-50",  text: "text-indigo-700",  dot: "bg-indigo-500",  border: "border-indigo-200"  },
  shipped:    { label: "Shipped",    bg: "bg-purple-50",  text: "text-purple-700",  dot: "bg-purple-500",  border: "border-purple-200"  },
  delivered:  { label: "Delivered",  bg: "bg-emerald-50", text: "text-emerald-700", dot: "bg-emerald-500", border: "border-emerald-200" },
  cancelled:  { label: "Cancelled",  bg: "bg-red-50",     text: "text-red-700",     dot: "bg-red-400",     border: "border-red-200"     },
};

export const getStatus = (s: string) =>
  STATUS_META[s] ?? { label: s, bg: "bg-slate-50", text: "text-slate-600", dot: "bg-slate-400", border: "border-slate-200" };
...
export const getDistanceKm = (fLat: number, fLng: number, tLat: number, tLng: number): number => {
  ...
  const dLat = r(tLat - fLat), dLng = r(tLng - fLng);
  const a = Math.sin(dLat / 2) ** 2 + Math.cos(r(fLat)) * Math.cos(r(tLat)) * Math.sin(dLng / 2) ** 2;
...
const getApiBase = (): string => {
  try { return (import.meta as any).env?.VITE_API_URL || "http://localhost:5000/api"; }
...
const resolveTotal = (p: unknown, fb: number): number => {
  ...
  const d = p as any;
  return typeof d.total === "number" ? d.total : typeof d.data?.total === "number" ? d.data.total : fb;
};
...
export function useOrders() {
  ...
  const buildQuery = useCallback((pageNum: number) => {
    ...
    q.set("page", String(pageNum)); q.set("limit", String(PAGE_LIMIT));
    ...
    else { if (filters.from) q.set("from", filters.from); if (filters.to) q.set("to", filters.to); }
    return q.toString();
  }, [search, filters]);
  ...
  const load = useCallback(async (pageNum = 1) => {
    ...
    try {
      ...
      setItems(list); setTotal(resolveTotal(payload, list.length)); setPage(pageNum);
  ...
  useEffect(() => {
    ...
    socket.on("admin:orderUpdated", (payload: { order?: Order }) => {
      ...
      setItems((prev) => {
        ...
        const next = [...prev]; next[idx] = { ...next[idx], ...incoming }; return next;
      });
  ...
  useEffect(() => { setFilters(readFilterParams(searchParams)); setPage(1); }, [searchParams]);
  ...
  const applyFilters = () => {
    ...
    filters.paymentMethod !== "all" ? p.set("paymentMethod", filters.paymentMethod) : p.delete("paymentMethod");
    if (filters.today) { p.set("today", "1"); p.delete("from"); p.delete("to"); }
    else { p.delete("today"); filters.from ? p.set("from", filters.from) : p.delete("from"); filters.to ? p.set("to", filters.to) : p.delete("to"); }
    setSearchParams(p);
  };
  ...
  const openDetail = async (orderId: string) => {
    ...
    try {
      ...
      setDetail(order); setEditStatus(order.status || ""); setEditRider(order.assignedRiderId?._id || "");
    } catch { alert("Failed to load order detail"); }
  ...
  const stats = {
    total, revenue: items.reduce((s, o) => s + (o.totalAmount || 0), 0),
  ...
  return {
    items, total, page, loading, search, setSearch,
    filters, setFilter, applyFilters, clearFilters,
    riders, stats,
    detail, detailLoading, openDetail, closeDetail,
    editStatus, setEditStatus, editRider, setEditRider,
    updating, updateStatus, assignRider,
    hasPerm, load,
