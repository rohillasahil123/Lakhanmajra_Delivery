...
export default function Categories() {
  ...
  const create = async () => {
    const parsedSubCategories = showCreateSubCategory
      ? parseSubCategoryNames(subCategoriesInput)
      : [];
    ...
    try {
      ...
      await load();
      if (parsedSubCategories.length > 0) {
        console.info('Category + sub-categories created', {
          subCategoryCount: parsedSubCategories.length,
        });
        setToast({
          message: `Category/Sub-categories created (${parsedSubCategories.length})`,
          type: 'success',
        });
      } else {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      await load();
      if (parsedSubCategories.length > 0) {
        console.info('Category updated + sub-categories added', {
          categoryId: editingId,
          subCategoryCount: parsedSubCategories.length,
        });
        setToast({
          message: `Category updated + ${parsedSubCategories.length} sub-categories added`,
          type: 'success',
        });
      } else {
      ...
      {hasPerm('categories:create') && (
        <div className="mb-4 bg-white p-4 rounded shadow">
          <div className="flex gap-2 items-center flex-wrap">
            ...
            />
            <button className="bg-sky-600 text-white px-4 rounded" onClick={create}>
              Create
            </button>
          </div>
          {createImagePreview && (
                  ...
                  <tr key={c._id} className="border-b last:border-0">
                    <td className="p-3">
                      {c.image ? (
                        ...
                        <div className="w-10 h-10 rounded border border-slate-200 bg-slate-50 text-slate-400 text-xs flex items-center justify-center">
                          No
                        </div>
                      )}
                    </td>
                          ...
                          <>
                            ...
                            >
                              ...
                              {items
                                .filter(
                                  (category) =>
                                    !getParentCategoryId(category) && category._id !== c._id
                                )
                                .map((category) => (
                                  <option key={category._id} value={category._id}>
                                    {category.name}
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    setEditRemoveImage(false);
                                    setEditImagePreview(
                                      file ? URL.createObjectURL(file) : c.image || ''
                                    );
                                  }}
                                />
                                {editImagePreview && (
