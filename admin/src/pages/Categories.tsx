...
export default function Categories() {
  ...
  const create = async () => {
    ...
    try {
      await api.post('/categories', { name, icon, priority });
      setName(''); setIcon(''); setPriority(0);
      await load();
  ...
  const saveEdit = async () => {
    ...
    try {
      await api.patch(`/categories/${editingId}`, { name: editName, icon: editIcon, priority: editPriority });
      setEditingId(null);
      await load();
  ...
  return (
    <div className="p-6">
      ...
      {hasPerm('categories:create') && (
        <div className="mb-4 bg-white p-4 rounded shadow flex gap-2 items-center">
          <input className="border px-3 py-2 rounded" placeholder="name" value={name} onChange={e => setName(e.target.value)} />
          <input className="border px-3 py-2 rounded" placeholder="icon (url)" value={icon} onChange={e => setIcon(e.target.value)} />
                  ...
                  <td className="p-3 flex gap-2">
                    {editingId === c._id ? (
                      <>
                        ...
                        <button className="px-2 py-1 bg-gray-200 text-sm rounded" onClick={() => setEditingId(null)}>Cancel</button>
                      </>
                    ) : (
                      <>
