...
function CreateRoleModal({
  ...
  const handleCreate = async () => {
    ...
    try { await onCreate(name.trim(), desc.trim(), perms); onClose(); }
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              <label className="text-xs text-slate-500 mb-1 block font-medium">Role Name *</label>
              <input className="border border-slate-200 px-3 py-2.5 rounded-xl w-full text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition"
                placeholder="e.g. Manager, Vendor..." value={name} onChange={(e) => setName(e.target.value)} />
            </div>
            <div className="col-span-2">
              <label className="text-xs text-slate-500 mb-1 block font-medium">Description</label>
              <input className="border border-slate-200 px-3 py-2.5 rounded-xl w-full text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition"
                placeholder="What can this role do?" value={desc} onChange={(e) => setDesc(e.target.value)} />
            </div>
          </div>
...
function RoleDetailModal({
  role, colorIdx, availablePermissions, hasPerm,
  onClose, onUpdate, onDelete,
}: {
  role: Role; colorIdx: number;
  availablePermissions: Permission[];
  hasPerm: (p: string) => boolean;
  onClose: () => void;
  onUpdate: (roleId: string, updates: any) => Promise<void>;
  ...
  const handleSave = async () => {
    if (!editName.trim()) return setError('Name required');
    setSaving(true); setError('');
    try {
      await onUpdate(role._id, { name: editName.trim(), description: editDesc.trim(), permissionIds: editPerms });
      setIsEditing(false);
        ...
        <div className={`px-6 py-5 rounded-t-2xl ${c.bg} border-b ${c.border}`}>
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                {isEditing ? (
                  <input className="border border-slate-300 px-2 py-1 rounded-lg text-sm font-semibold w-40 bg-white"
                    value={editName} onChange={(e) => setEditName(e.target.value)} />
                ) : (
                  <h3 className="font-bold text-slate-800 text-lg leading-tight capitalize">{role.name}</h3>
                )}
                {isEditing ? (
                  <input className="border border-slate-300 px-2 py-1 rounded-lg text-xs mt-1 w-56 bg-white"
            ...
            <button onClick={onClose} className="w-8 h-8 rounded-full bg-white/70 hover:bg-white flex items-center justify-center text-slate-500 transition-colors">✕</button>
          </div>
          ...
          {activeTab === 'permissions' && (
            <div className="space-y-3">
                  ...
                  <div key={module} className="border border-slate-100 rounded-xl overflow-hidden">
                    <div className="flex items-center justify-between px-3 py-2 bg-slate-50">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-slate-400">
                          {isEditing ? `${selectedInEdit.length}/${modulePerms.length}` : `${grantedInRole.length}`}
                        </span>
                      </div>
                      {isEditing && (
                        <button type="button" className="text-xs text-blue-500 hover:text-blue-700 transition-colors"
                          onClick={() => {
                            ...
                            if (allSel) setEditPerms((prev) => prev.filter((id) => !modulePerms.find((p) => p._id === id)));
                            else setEditPerms((prev) => [...new Set([...prev, ...modulePerms.map((p) => p._id)])]);
                          }}>
                    ...
                    <div className="px-3 py-2 flex flex-wrap gap-2">
                      {(isEditing ? modulePerms : grantedInRole).map((p) => {
                        ...
                        return isEditing ? (
                          <label key={p._id} className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium cursor-pointer border transition-all ${
                            active ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300'
                          }`}>
                            <input type="checkbox" className="hidden" checked={active} onChange={() => togglePerm(p._id)} />
                            {action}
                          </label>
                        ) : (
                          <span key={p._id} className={`px-2.5 py-1 rounded-full text-xs font-medium border ${c.icon} ${c.border}`}>
                            {action}
                          </span>
                        );
                      })}
              ...
              {!isEditing && (!role.permissions || role.permissions.length === 0) && (
                <div className="text-center py-8 text-slate-400 text-sm">No permissions assigned to this role</div>
              )}
            </div>
          )}
            ...
            <div>
              {!usersLoaded ? (
                ...
                <div className="space-y-2">
                  {users.map((u) => (
                    <div key={u._id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                      ...
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${u.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
          ...
          {hasPerm('roles:manage') ? (
            isEditing ? (
              <div className="flex gap-2 w-full">
                <button className="px-4 py-2 bg-slate-200 text-slate-700 rounded-xl text-sm hover:bg-slate-300 transition-colors"
                  onClick={() => { setIsEditing(false); setEditName(role.name); setEditDesc(role.description || ''); setEditPerms(role.permissions?.map((p) => p._id) || []); setError(''); }}>
                  Cancel
                </button>
                <button className="px-5 py-2 bg-blue-600 text-white rounded-xl text-sm hover:bg-blue-700 disabled:opacity-50 font-medium transition-colors flex-1"
              ...
              <div className="flex gap-2 w-full">
                <button className="px-4 py-2 bg-red-50 text-red-600 border border-red-200 rounded-xl text-sm hover:bg-red-100 transition-colors"
                  onClick={async () => { if (confirm(`Delete role "${role.name}"?`)) { await onDelete(role._id); onClose(); } }}>
                  Delete
                </button>
                <button className="px-4 py-2 bg-slate-200 text-slate-700 rounded-xl text-sm hover:bg-slate-300 transition-colors ml-auto"
                  onClick={onClose}>Close</button>
                <button className="px-5 py-2 bg-blue-600 text-white rounded-xl text-sm hover:bg-blue-700 font-medium transition-colors"
            ...
            <button className="px-4 py-2 bg-slate-200 text-slate-700 rounded-xl text-sm ml-auto" onClick={onClose}>Close</button>
          )}
        </div>
      </div>
    </div>
...
function RoleCard({ role, colorIdx, onClick }: {
  role: Role; colorIdx: number; onClick: () => void;
}) {
  const c       = ROLE_COLORS[colorIdx % ROLE_COLORS.length];
      ...
      {modules.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-3">
          {modules.slice(0, 4).map((mod) => (
            <span key={mod} className="inline-flex items-center gap-0.5 text-xs text-slate-500 bg-white/80 border border-slate-200 px-2 py-0.5 rounded-full">
...
export default function Roles() {
  ...
  const handleCreate = async (name: string, desc: string, permIds: string[]) => {
    const res     = await api.post('/admin/roles', { name, description: desc, ...(permIds.length ? { permissionIds: permIds } : {}) });
    const created = res.data?.data ?? res.data;
    setRoles((r) => [created, ...r]);
  };

  const handleUpdate = async (roleId: string, updates: any) => {
    ...
    setSelectedRole((prev) => prev && prev.role._id === updated._id ? { ...prev, role: updated } : prev);
  };
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">{roles.length} roles · {allPerms.length} permissions available</p>
        </div>
        {hasPerm('roles:manage') && (
          <button
        ...
        <div className="text-center py-20">
          ...
          {hasPerm('roles:manage') && (
            <button className="px-4 py-2 bg-blue-600 text-white rounded-xl text-sm hover:bg-blue-700 transition-colors"
              onClick={() => setShowCreate(true)}>+ Create First Role</button>
          )}
        </div>
      ) : (
